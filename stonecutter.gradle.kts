...
publishMods {
	...
	if (shouldAnnounceDiscord) {
		...
		discord("publishDiscordBody") {
			...
			if (!isDev) {
				content = changelog.map { "# [FastScrolling version `$version` is out!](https://modrinth.com/mod/fastscrolling/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n"  }
			}
			else {
				content = changelog.map { "# [FastScrolling version `$version` is out!](https://modrinth.com/mod/fastscrolling-dev/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n" }
			}
